...
@HiltViewModel
class EPDSStatisticsViewModel @Inject constructor(
    private val questionnaireRepository: QuestionnaireRepository,
    protected val newbornInfoRepository: NewbornInfoRepository
) : BaseQuestionnaireStatisticsViewModel(questionnaireRepository, newbornInfoRepository) {

    @RequiresApi(Build.VERSION_CODES.O)
    override suspend fun fetchUserAnswersFromFirestore(): Map<String, Answer> {
        return try {
            val pregnancyInfoList = newbornInfoRepository.getUserInfo().firstOrNull()
            ...
            val anxietyQuestionnaireResults = pregnancyInfo.questionnaireResults
